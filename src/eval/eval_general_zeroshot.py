    ...
    def evaluate(self):
        ...
        for item in self.data:
            ...
            message = [
                # {"role": "system", "content": SYSTEM_PROMPT},
                {
                    ...
                    "content": [
                        {"type": "video"},
                        {"type": "text", "text":self.question_template.format(Question=question)+ TYPE_TEMPLATE.get(problem_type, TYPE_TEMPLATE["multiple choice"])}
                    ]
                }
            ]
        ...
        while i < total_samples:
            try:
                ...
                formatted_messages = []
                video_paths = []
                for item, question in zip(batch_data, prompt_questions[i:batch_end]):
                    formatted_message = [{
                        "role": "user",
                        "content": [
                            {"type": "video"},
                            {"type": "text", "text": self.question_template.format(Question=question)},
                        ],
                    }]
...
def evaluate(model_name,dataset_name,batch_size):
    ...
    private_data_root = os.environ.get("PRIVATE_DATA_ROOT", ".")
    dataset_name = dataset_name
    
    # Construct model path
    model_path = model_name if os.path.isabs(model_name) else os.path.join(private_data_root, model_name)
    base_path = model_path.split('/checkpoint-')[0].split('\\checkpoint-')[0]
    ...
    default_dataset_path = os.path.join(private_data_root, "data", "evaluation", f"{dataset_name}.json")
    if not os.path.exists(default_dataset_path):
        default_dataset_path = os.path.join(private_data_root, "evaluation", f"{dataset_name}.json")
    dataset_path = os.environ.get("EVAL_DATASET_PATH", default_dataset_path)

    output_root = os.environ.get("EVAL_OUTPUT_ROOT", ".")
    model_output_name = os.path.basename(base_path.rstrip("/\\"))
    output_path = os.path.join(
        output_root,
        "logs",
        dataset_name,
        "test",
        f"{model_output_name}_zeroshot",
        f"{os.path.basename(model_path)}_{dataset_name}_zeroshot_eval.json",
    )

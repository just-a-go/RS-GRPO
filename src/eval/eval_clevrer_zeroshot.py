...
def evaluate(model_name, batch_size):
    ...
    try:
        ...
        dataset_path = os.environ.get(
            "CLEVRER_VAL_PATH",
            os.path.join(private_data_root, "data", "CLEVRER", f"{dataset_name}.json"),
        )
        output_root = os.environ.get("EVAL_OUTPUT_ROOT", ".")
        model_output_name = os.path.basename(base_path.rstrip(os.sep))
        output_path = os.path.join(
            ...
            f"{os.path.basename(model_path)}_zeroshot_eval.json",
        )
